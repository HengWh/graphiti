...
if not gemini_base_url or not openai_base_url or not api_key or not neo4j_uri or not neo4j_user or not neo4j_password or not database:
    raise ValueError(
...
gemini_config = GeminiEmbedderConfig(
    ...
    base_url=gemini_base_url,  # 从环境变量获取 Gemini base URL
...
openai_client_config = LLMConfig(
    api_key=api_key,  # 从环境变量获取 Gemini API key
    base_url=openai_base_url  # 从环境变量获取 openai base URL
)
openai_cross_encoder=OpenAIRerankerClient(config=openai_client_config)
...
app = Graphiti(neo4j_uri, neo4j_user, 
               ...
               cross_encoder=openai_cross_encoder)
